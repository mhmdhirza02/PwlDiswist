...
@section('header', 'Tambah Destinasi Wisata')
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
            <h3 class="font-bold text-slate-800 text-sm italic items-center flex gap-2 not-italic">
                ...
                Formulir Destinasi Baru
            </h3>
        </div>

        <form method="POST" action="/admin/wisata" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="deskripsi" required rows="4"
                          class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-sm"
                          placeholder="Berikan deskripsi singkat tentang tempat ini..."></textarea>
            </div>

            <!-- Upload Foto Sampul -->
            <div class="space-y-1.5">
                <label class="text-xs font-bold text-slate-600 uppercase tracking-wider">Foto Sampul</label>
                <div class="p-4 border-2 border-dashed border-slate-200 rounded-lg bg-slate-50 flex flex-col items-center justify-center gap-2 relative group hover:bg-slate-100 transition-colors">
                    <input type="file" name="foto" required class="absolute inset-0 opacity-0 cursor-pointer" id="fotoInput">
                    <i class="bi bi-image text-slate-400 text-2xl"></i>
                    <p class="text-xs text-slate-500 font-medium" id="fileNameDisplay">Klik atau seret foto sampul ke sini</p>
                </div>
            </div>
