...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Daftar Kuliner</h1>
            <p class="text-sm text-slate-500">Manajemen konten kuliner lezat yang ada di Yogyakarta.</p>
        </div>
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-slate-50 border-b border-slate-200 text-slate-600 uppercase text-[11px] font-bold tracking-wider">
                    <tr>
                        ...
                        <th class="px-4 py-4">Nama Kuliner</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach($kuliners as $index => $k)
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-6 py-4 text-slate-400 font-medium">{{ $index + 1 }}</td>
                        <td class="px-4 py-4">
                            @if($k->image)
                                <img src="{{ asset('storage/'.$k->image) }}" class="w-16 h-10 object-cover rounded shadow-sm" alt="{{ $k->name }}">
                            @else
                        ...
                        <td class="px-4 py-4 font-bold text-slate-800">{{ $k->name }}</td>
                        <td class="px-4 py-4 text-slate-600 font-medium">{{ $k->category }}</td>
                        <td class="px-4 py-4 font-bold text-primary">{{ $k->price_range }}</td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <a href="/admin/kuliner/{{ $k->id }}/edit" 
                                ...
                                <form method="POST" action="/admin/kuliner/{{ $k->id }}" class="inline" onsubmit="return confirm('Hapus data ini?')">
                                    @csrf @method('DELETE')
